...
def create_stock_image_prompts(d: dict) -> list:
    ...
    return [
        {"idx": 1, "title": "카드 1 — 목표주가 + 상승여력 (메인 비주얼)",
         "prompt": (
             f"{base}\n"
             f"레이아웃 (위→아래):\n"
             f"① 상단 소형: '{broker}' 증권사 텍스트 (회색)\n"
             f"② 종목명 '{name}' 굵은 흰색 대형 제목\n"
             f"③ 빨간 배지: '{opinion}'\n"
             f"④ [핵심 비주얼] 중앙에 큰 섹션:\n"
             f"   - '목표주가' 라벨(소형 회색) 위에\n"
        ...
        {"idx": 3, "title": "카드 3 — 요약 + 구독 CTA",
         "prompt": (
             f"{base}\n"
             f"상단: '리포트읽어드림' 채널명 (회색 소형)\n"
             f"중앙: '{name}' 굵은 흰색\n"
             f"목표주가 '{target}' + 상승여력 '{upside}' 강조 박스\n"
             f"하단: '구독하고 다음 리포트도 받아가세요!' (빨간 텍스트)"
         )},
    ]
